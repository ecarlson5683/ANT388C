...
```{r}
# using {base} R (this version did not work for me)
...
d[1:10, ] # 10 most dense

d <- d[order(d$density), ]
d[1:10] # 10 least dense
```
...
mean(s$population, na.rm = TRUE) # to get mean specifically
...
With `makeDataReport()` from the {dataMaid} package, you can generate a nicely formatted report that gets written out to a location of your choice. Again, the output of this code is not shown as it generates a report in an external file.

```{r}
#| eval: false
library(dataMaid)
# this code below produces a formatted report, with the type of report
# specified by `output=`
makeDataReport(d,
  output = "html",
...
barplot(d$log_population, xlab = "Case", ylab = "log(Population)")
dotchart(d$log_population, xlab = "log(Population)", ylab = "Case")

boxplot(d$log_area, ylab = "log(Area)")
barplot(d$log_area, xlab = "Case", ylab = "log(Area)")
dotchart(d$log_area, xlab = "log(Area)", ylab = "Case")
```
...
# to view multiple IVs or factors (w/ same DV) in the same graph
...
```{r}
glimpse(d) # basic info of groups

# nicer version with five number summary and histograms
...
```{r}
# boxplot of female body mass by family in {base R}
boxplot(log(d$Body_mass_female_mean) ~ d$Family,
        xlab = "Family",
        ylab = "log(Female Body Mass)",
        col = "lightblue")

# boxplot of female body mass by family in {ggplot2}
p <- ggplot(data = d, 
...
p <- ggplot(data=d, aes(x=log(Body_mass_female_mean),
                        ...
                        color = factor(Family)
                        )) + # build a plot object and color points by Family
  xlab("log(Female Body Mass)") + 
  ylab("log(Female Brain Size)") + # modify the axis labels
  geom_point(na.rm=TRUE) + # make a scatterplot
  theme(legend.position="bottom", legend.title=element_blank()) # modify the legend
p # plot the object
```
...
p <- ggplot(data=d, aes(x=log(MaxLongevity_m),
  ...
  xlab("log(Maximum Longevity (m))") + 
  ylab("log(Female Body Mass)") + # modify the axis labels
  geom_point(na.rm=TRUE) + # make a scatterplot
...
We can also use either the `pairs()` function from {base} ***R***, the `scatterplotMatrix()` function from the {car} package, or the `pairs.panel()` function from the {psych} package to do multiple scatterplots simultaneously. The `ggpairs()` function from the {GGally} package can also be used, but it is slower.
...
> NOTE: This is the one I would use / have seen published
...
p1 <- ggplot(data=d, 
  ...
  xlab("log(Female Body Size)") + ylab("log(Lifespan)")

p2 <- ggplot(data=d, 
             aes(x=log(Body_mass_male_mean),
                 y=log(MaxLongevity_m))) + 
  geom_point(na.rm = TRUE) + 
  geom_smooth(method="lm", na.rm = TRUE) + 
  xlab("log(Male Body Size)") + ylab("log(Lifespan)")
...
p3 <- ggplot(data=d, 
  ...
  geom_boxplot(na.rm=TRUE) + 
  theme(axis.text.x=element_text(angle=90)) + 
  xlab("Family") + ylab("log(Female Body Mass)")
```
...
How to make the C not overlap with the y-axis label?
